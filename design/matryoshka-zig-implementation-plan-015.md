# Matryoshka Zig 0.16 — Staged Implementation Plan (015)
...
- After each completed stage, create a new plan version (e.g., plan-014 → plan-015).
...
### Stage 7.b — Event Sources: Examples

**Purpose**: show Select/Future APIs in real usage patterns.

**All Stage 7 scenarios are examples** (per task2-tests-001.md: "Remaining task2 scenarios (17-61) are all examples").

**Scenarios**: task2 scenarios 25-31, 42-56.

**Prologue**: ask owner whether he wants an audit before Stage 7.b.

**New files** (`examples/layer4/`)

| File | Scenario | Story |
|------|----------|-------|
| `select_two_mailboxes.zig` | 25 | Two mailboxes + timer; timer fires first; re-spawn event source |
| `select_cancel_close.zig` | 26 | Timer cancel → close both mailboxes → walk remaining |
| `select_cancel_master_decides.zig` | 27 | Cancel → master closes one, re-spawns other |
| `select_mixed_sources.zig` | 28 | Mailbox + pool + timer in one Select |
| `select_cancel_recycle.zig` | 29 | Cancel → close → pool.put_all recycles items |
| `mailbox_timeout.zig` | 30 | mailbox.receive with timeout → Io.sleep retry |
| `select_graceful_shutdown.zig` | 31 | Select cancel, mixed cancellation points, no item loss |
| `select_mailbox_event.zig` | 42 | Single mailbox + timer; re-spawn after each item |
| `select_direct_push.zig` | 43 | Direct push from wild thread (replaces socket story — see below) |
| `select_mailbox_close.zig` | 44 | mailbox.close propagates .closed into Select |
| `select_mailbox_cancel.zig` | 45 | select.cancel propagates .canceled through mailbox |
| `select_pool_event.zig` | 46 | Pool availability as Select event source |
| `select_job_pool.zig` | 47 | Job pool pattern — worker puts back, master re-spawns |
| `select_mailbox_pool_timer.zig` | 48 | Inbox + pool + timer in one Select |
| `receive_future_direct.zig` | 49 | receive_future awaited directly (no Select) |
| `get_wait_future_direct.zig` | 50 | get_wait_future awaited directly |
| `receive_future_timeout.zig` | 51 | receive_future with timeout → .timeout |
| `future_single_threaded.zig` | 52 | receive_future on global_single_threaded → ConcurrencyUnavailable |
| `pool_fan_in.zig` | 53 | 3 workers put, one master gets all 3 |
| `pool_fan_out.zig` | 54 | Master seeds N, 3 workers each get one |
| `producer_consumer_recycle.zig` | 55 | Pool → producer → mailbox → consumer → pool |
| `job_pool_circular.zig` | 56 | Circular flow; pool.get_wait_select drives pace |

**Scenario 43 — direct push from wild thread** (new pattern, replaces socket story):

```
// Request/response via direct push — ownership circuit:
//
//  CappedPool
//      │  pool.get
//      ▼
//   Master ──mailbox.send──► Wild thread (std.Thread)
//      ▲                         │ process
//      │                         │ putOneUncancelable
//      │    Select(.response)    ▼
//      └────────────────── select.queue
...
//   pool.put (recycle)
//      │
//  CappedPool  ◄── cap blocks pool.get until item returns
//
// Cap = N limits in-flight requests. No item in pool → master blocks.
```

Shutdown: master closes pool → wild thread's `mailbox.receive` wakes with `error.Closed` → thread exits → master joins.

**Test wrappers**: `tests/layer4_select.zig` — one wrapper per example, `Io.Threaded.init(gpa, .{})`.

Add `@import("layer4_select.zig")` to `tests/matryoshka_tests.zig`.

Update `examples/layer4/layer4.zig` re-exports.

**Every new example must have an ASCII ownership diagram** (per Examples MUST rule).

**Execution checklist**

- [ ] Step 0 — Prologue: audit or skip?
- [ ] Step 1 — Write all 22 example files (table above) with ownership diagrams.
- [ ] Step 2 — Write `tests/layer4_select.zig` (22 test wrappers).
- [ ] Step 3 — Wire into `tests/matryoshka_tests.zig` and `examples/layer4/layer4.zig`.
- [ ] Step 4 — `kitchen/build_and_test_debug.sh` — pass.
- [ ] Step 5 — `kitchen/build_and_test_all.sh` — pass (all 4 modes).
- [ ] Step 6 — `kitchen/build_cross_debug.sh` — pass.
- [ ] Step 7 — Post-stage cleanup.
- [ ] Step 8 — AI-sh + banned words scan.
- [ ] Step 9 — Update `design/STATUS.md`.
- [ ] Step 10 — Create `design/matryoshka-zig-implementation-plan-016.md`.

**Checkpoint**
- All 22 examples build and pass their test wrappers.
- All kitchen scripts pass.
- Every example has an ASCII diagram.
