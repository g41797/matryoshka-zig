...
- 150 scenarios are the test plan: 89 in task1, 61 in task2.
...
- `task1-scenarios-001.md` — 89 scenarios (Layers 1-3) — historical source
...
- After each completed stage, create a new plan version (e.g., plan-005 → plan-006).
...
**What to build**: examples 53-59 (task1-scenarios-001.md Layer 2 Examples), each with a test wrapper.

**Files**:
- `examples/block2/block2.zig` — re-exports all mailbox examples
- `examples/block2/simple_send_receive.zig` — scenario 53
- `examples/block2/worker_loop.zig` — scenario 54
- `examples/block2/oob_signal.zig` — scenario 55
- `examples/block2/pipeline.zig` — scenario 56
- `examples/block2/request_response.zig` — scenario 57
- `examples/block2/fan_in.zig` — scenario 58
- `examples/block2/shutdown_cleanup.zig` — scenario 59
...
**Scenarios 56, 58** (pipeline, fan-in) need real threads — `std.Thread.spawn` (Open Item 10).
...
**Scenarios to verify**: task1-scenarios-001.md Layer 3 Tests (60-85).
...
- All 150 scenarios green (tests + examples).
...
Totals: 89 task1 (Stages 1-3), 61 task2 (Stages 5-8).
