# Collected Context for Matryoshka Zig Implementation (v002)
...
- Slot ownership semantics (null-after-send) [was MayItem, renamed Proposal 27]
...
- Caller still owns handle (Slot not cleared) — handle not lost
...
**Result by value**: Adapters return handles inside the result union, not through `*Slot` pointers. No cross-thread pointer hazards.
...
- Synchronous `mailbox.receive` takes `m: *Slot` — pointer to caller's stack variable
- In async case, concurrent task writes through this pointer from a different thread
- Solution: result carries handle by value inside tagged union — no `*Slot` crosses thread boundary
- Adapter creates local `Slot`, calls blocking API, packages result into union
...
| 27 | Slot/NodeHandle rename | `MayItem` → `Slot`, `*PolyNode` → `NodeHandle`. `MailboxHandle = NodeHandle`, `PoolHandle = NodeHandle`. Visual ownership model added to API reference intro. Matryoshka transports handles, not "items" |

Note - 24 applied (2026-06-22): all "DLL" abbreviations replaced with full name or "linked list"
Note - 27 applied (2026-06-23): `MayItem` → `Slot`, `*PolyNode` → `NodeHandle` throughout API reference
