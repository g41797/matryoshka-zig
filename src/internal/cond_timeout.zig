...
pub fn condition_waitTimeout(cond: *Condition, io: Io, mutex: *Mutex, timeout: Io.Timeout) WaitTimeoutError!void {
    const deadline: Io.Timeout.Deadline = timeout.toDeadline(io);

    var epoch: u32 = cond.epoch.load(.acquire);

    {
        const prev_state: @TypeOf(cond.state).Raw = cond.state.fetchAdd(.{ .waiters = 1, .signals = 0 }, .monotonic);
    ...
    while (true) {
        const result: Io.FutexWaitResult = io.futexWaitTimeout(u32, &cond.epoch.raw, epoch, deadline);

        epoch = cond.epoch.load(.acquire);

        {
            var prev_state: @TypeOf(cond.state).Raw = cond.state.load(.monotonic);
        ...
        result catch |err| {
            const prev_state: @TypeOf(cond.state).Raw = cond.state.fetchSub(.{ .waiters = 1, .signals = 0 }, .monotonic);
